...
static void attach_metadata_object (GstDsExample * dsexample,
    NvDsObjectMeta * obj_meta, DsExampleOutput * output,float i);
...
/* apply gaussian blur to the detected objects */
  GaussianBlur(in_mat(crop_rect), in_mat(crop_rect), cv::Size(15,15), 14);
 ...
 */
	...
	gst_dsexample_transform_ip (GstBaseTransform * btrans, GstBuffer * inbuf)
	{
	  ...
	  if (batch_meta == nullptr) {
		...
		/*ENTRO NELLA MODALITA FULL FRAME*/

	  if (dsexample->process_full_frame) {
	g_print("The custom plugin is doing something....\n");
/*batch_meta->frame_meta_list è una lista di puntatori a NvDsFrameMeta del batch corrente. NvDsFrameMeta è una struttura contenente molte informazioni su ogni frame: 
*/

/*l_frame Holds information about given metadata pool.*/
	    ...
	    for (l_frame = batch_meta->frame_meta_list; l_frame != NULL;
	      l_frame = l_frame->next)
	    {
/*Accedo a questo ramo ogni frame!!*/
	      frame_meta = (NvDsFrameMeta *) (l_frame->data);
	      cv::Mat in_mat;
/*entro nel ramo per la sfocatura degli oggetti*/
	      if (dsexample->blur_objects) {
		/* Map the buffer so that it can be accessed by CPU */
...
int i = 1;
	      ...
	      {
NvDsMetaList *l_objINIT=frame_meta->obj_meta_list;
/*Dentro questo ciclo vengono passati uno per uno gli oggetti rilevati dal plugin sovrastante (nvinfer)*/
		obj_meta = (NvDsObjectMeta *) (l_obj->data);
		
	     // obj_meta è l'oggetto riconosciuto!!!
		if (dsexample->blur_objects) {
...
float y = 0.0;
float distanza=0.0; 
NvDsMetaList *l_objprefor=l_obj;
//l_obj=oggetto attuale

while(l_obj!= NULL){
	while(l_objINIT!= NULL){
		if(l_objINIT!=l_obj){
			x = (((NvDsObjectMeta *)l_obj ->data)-> rect_params).top / 2 - (((NvDsObjectMeta *)l_objINIT ->data)-> rect_params).top / 2;
        		y = (((NvDsObjectMeta *)l_obj ->data)-> rect_params).left / 2 - (((NvDsObjectMeta *)l_objINIT ->data)-> rect_params).left / 2;
			distanza=SqrtNumber(x * x + y * y);
		}
		l_objINIT = l_objINIT -> next;
	}
	l_obj = l_obj -> next;
}
l_obj=l_objprefor;
		...
		attach_metadata_object (dsexample, obj_meta, output,distanza);
...
attach_metadata_object (GstDsExample * dsexample, NvDsObjectMeta * obj_meta,
    DsExampleOutput * output,float i)
{
  ...
  if (text_params.display_text) {
    gchar *conc_string = g_strconcat ("ciao","swago", NULL);
...
g_print("\nDIST== %i",(int)i);
if((int)i==0){
    text_params.display_text = g_strconcat ("Distanza: ","None", NULL);
}else{
	
    text_params.display_text = g_strconcat ("Distanza: ",buffer, NULL);
}
    /* Font face, size and color */
